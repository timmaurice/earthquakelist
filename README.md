...
| `map_theme_mode`  | `string`   | `auto`  | Whether the map uses its light or dark style. `auto` follows your Home Assistant theme; `light` and `dark` pin it regardless, for dashboards that stay on one look.                                                                                                                  |
| `map_lock`        | `boolean`  | `false` | Start with map interaction locked, so dragging across the map scrolls the dashboard instead of panning it — useful on touch screens. The lock button on the map switches this for the current view; markers stay clickable either way.                                               |
