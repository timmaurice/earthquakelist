...
export default {
  ...
  plugins: [
    ...
    // No `mangle.properties`: MapLibre's main-thread code and its Web Worker talk to each other
    // over postMessage using plain object property names. The worker reaches the output as a
    // string (see `maplibreWorkerInlinePlugin` above), which Terser treats as opaque text and
    // leaves exactly as MapLibre built it, while a property mangler here would rewrite the live
    // main-thread code only. That would desync the two sides and break tile loading silently.
    !dev && terser(),
  ],
};
